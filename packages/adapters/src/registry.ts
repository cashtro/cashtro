...
import { createContrarianAdapter } from "./contrarian.js";
...
export function defaultAdapters(): Record<string, AgentAdapter> {
  ...
  return {
    ...
    contrarian: createContrarianAdapter(),
...
export function pickAdapter(runtime: string, adapters: Record<string, AgentAdapter> = defaultAdapters()): AgentAdapter {
  if (runtime === "local" && adapters.local) return adapters.local;
  if (runtime === "contrarian" && adapters.contrarian) return adapters.contrarian;
