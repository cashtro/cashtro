...
func initInvoke(k *kernel.Kernel, call kernel.Call) (kernel.Result, error) {
	...
	case "os.cycle":
		...
		k.Publish("init", "voltron", "cycle lancé avec instinct", map[string]any{"line": payloadQuery(call, "id")})
		return res, nil
	case "os.engine":
	...
	for _, step := range steps {
		...
		if !res.OK {
			return kernel.Result{OK: false, Message: res.Message, Data: map[string]any{
				"line": line, "directedBy": []string{"voltron", "instinct"}, "steps": done,
			}}, nil
		}
	}
	k.Publish("manager", "instinct", "cycle bouclé "+line, map[string]any{"steps": len(done)})
