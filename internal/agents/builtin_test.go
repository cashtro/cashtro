...
func TestBootLoadsAllAgentics(t *testing.T) {
	...
	if got := status["lines"]; got != 13 {
		t.Fatalf("board lines = %v, want 13", got)
	}
	...
	org := Chart()
	if len(org.Seats) != 3 || len(org.Departments) != 9 || len(org.Divisions) != 10 {
	...
	res, err = k.Invoke(context.Background(), "manager", kernel.Call{Capability: "manager.ledger"})
	if err != nil || !res.OK {
		t.Fatalf("manager.ledger: %+v %v", res, err)
	}
	moves, ok := res.Data.([]Move)
