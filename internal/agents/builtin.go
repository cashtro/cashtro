...
func Builtins(cat *catalog.Catalog, router *model.Client) []kernel.Agent {
	return []kernel.Agent{
		...
		resident(kernel.Spec{
			ID: "explorer", Name: "Explorer", Kind: kernel.KindUser, Mode: kernel.ModeResident,
			Role: "search", Summary: "Finds code, services, and patterns across the mandate repos.",
		...
		resident(kernel.Spec{
			ID: "memory", Name: "Memory", Kind: kernel.KindUser, Mode: kernel.ModeResident,
			Role: "recall", Summary: "Keeps decisions, fixes, and house rules across sessions.",
			Capabilities: []string{"memory.recall"}, Autostart: true,
		}, nil),
		resident(kernel.Spec{
			ID: "comms", Name: "Comms", Kind: kernel.KindUser, Mode: kernel.ModeResident,
			Role: "signal", Summary: "Slack, mail, and support threads — outbound only after confirm.",
			Capabilities: []string{"comms.send"}, Autostart: true,
		}, nil),
		resident(kernel.Spec{
			ID: "planner", Name: "Planner", Kind: kernel.KindUser, Mode: kernel.ModeResident,
			Role: "backlog", Summary: "Turns a spec into a backlog the delivery line can walk.",
