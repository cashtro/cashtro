...
func seedResearch(k *kernel.Kernel) {
	seeds := []kernel.Note{
		...
		{Agent: "research", Source: "cashtro-roster", URL: "", Claim: "Processes wear builders — Lautaro, Builder, Creator, then the rest of the crew. Products keep their names — Casa Crypto and Prolifik stay products, never process titles.", Quote: "IDs and verbs stay stable. ScanApp, Proximity, Casa Crypto, Prolifik live on the delivery line."},
	}
	for _, n := range seeds {
		k.WriteNote(n)
