...
func New(k *kernel.Kernel) http.Handler {
	...
	mux.HandleFunc("POST /api/watch/build", s.watchBuild)
	...
	mux.HandleFunc("POST /api/companies/{id}/agents", s.addCompanyAgent)
	mux.HandleFunc("POST /api/companies/{id}/agents/{aid}/invoke", s.invokeCompanyAgent)
...
func (s *api) watchBuild(w http.ResponseWriter, r *http.Request) {
	s.invokeWatch(w, r, "watch.build")
}
