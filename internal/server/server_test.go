...
func TestOSAndAgents(t *testing.T) {
	...
	res = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodPost, "/api/comms", strings.NewReader(`{"body":"overnight outbound draft · no send until allow"}`))
	req.Header.Set("Content-Type", "application/json")
	h.ServeHTTP(res, req)
	if res.Code != http.StatusCreated || !strings.Contains(res.Body.String(), "parked confirm") {
		t.Fatalf("comms = %s", res.Body.String())
	}

	res = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodPost, "/api/mail", strings.NewReader(`{"from":"planner","to":"delivery","kind":"backlog","body":"overnight mail"}`))
	req.Header.Set("Content-Type", "application/json")
	h.ServeHTTP(res, req)
	if res.Code != http.StatusCreated || !strings.Contains(res.Body.String(), "overnight mail") {
